...
internal sealed class NamePlateGui : IInternalDisposableService, INamePlateGui
{
    ...
    {
        var token = new CallOnceToken(this.onRequestedUpdateHook.Original, addon, numberArrayData, stringArrayData);
        ...
        try
        {
            ...
            else
            {
                ...
                else if (updatedHandlers.Count != 0)
                {
                    ...
                    if (this.context.HasParts)
                        this.ApplyBuilders(updatedHandlers.ToArray().AsSpan());
                    token.Call();
    ...
    private unsafe ref struct CallOnceToken(
        AtkUnitBase.Delegates.OnRequestedUpdate original,
        AtkUnitBase* addon,
        ...
        StringArrayData** stringArrayData)
    {
        ...
        internal void Call()
        {
            ...
            try
            {
                original.Invoke(addon, numberArrayData, stringArrayData);
            }
            catch (Exception e)
            {
